...
scoreboard players remove @s sandstone_summit_booth.npc.dialogue.reveal_cut 53
data modify storage sandstone_summit_booth:npcs_reveal full set value ' all the way down.'
...
data modify storage sandstone_summit_booth:npcs_reveal runs set value [{text:'This whole booth?\nJust lines of '},{text:'code',color:'light_purple'},{text:".\nIt's "},{text:'TypeScript',color:'blue'},{text:''}]
data modify storage sandstone_summit_booth:npcs_reveal runs[4].text set from storage sandstone_summit_booth:npcs_reveal cut
execute at @s run function sandstone_summit_booth:sections/npcs/dialogue/glitchy/render/switch2/case_0/if5/execute_at