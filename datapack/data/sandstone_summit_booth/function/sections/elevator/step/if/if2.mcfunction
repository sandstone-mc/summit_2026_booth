execute as @e[type=minecraft:block_display, limit=1, tag=sandstone_summit_booth.elevator.car] at @a[tag=sandstone_summit_booth.elevator.driver, limit=1, gamemode=!spectator] run tp @s -55 ~-0.5 46
execute as @a[tag=sandstone_summit_booth.elevator.driver, limit=1, gamemode=!spectator] run function sandstone_summit_booth:sections/elevator/step/if/if2/execute_as
