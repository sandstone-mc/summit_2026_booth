...
for (let s = 0; s < songCount; s++) {
	...
	const nsPrefix = `${NAMESPACE}:sections/rythm/songs/${safeName}`
	...
	for (let seg = 0; seg < segmentCount; seg++) {
		...
		const fnName = `sections/rythm/songs/${safeName}/seg${seg}`
	...
	const playFn = MCFunction(`sections/rythm/songs/${safeName}/play`, () => {
	...
	const stopFn = MCFunction(`sections/rythm/songs/${safeName}/stop`, () => {
	...
	for (let bi = 0; bi < chart.beatTicks.length; bi++) {
		...
		const fnName = `sections/rythm/songs/${safeName}/w${beatTick}`
	...
	for (const [eventIdx, stepTicks] of chart.parkourStepTicks.entries()) {
		for (let si = 0; si < stepTicks.length; si++) {
			const step = si
			const fnName = `sections/rythm/songs/${safeName}/pk${eventIdx}_s${step}`
		...
		const cleanupFnName = `sections/rythm/songs/${safeName}/pk${eventIdx}_end`
	...
	const wallFn = MCFunction(`sections/rythm/songs/${safeName}/walls`, () => {
	...
	const wallStopFn = MCFunction(`sections/rythm/songs/${safeName}/walls_stop`, () => {
...
export const playSong = MCFunction('sections/rythm/songs/play', () => {
...
export const stopSong = MCFunction('sections/rythm/songs/stop', () => {
	for (const fn of songStopFns) fn()
}, { lazy: true })

export const scheduleWalls = MCFunction('sections/rythm/songs/schedule_walls', () => {
...
export const stopWalls = MCFunction('sections/rythm/songs/stop_walls', () => {
