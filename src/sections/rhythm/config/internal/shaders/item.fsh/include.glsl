#define SND_STAR_ITERATIONS 10
#define SND_STAR_FORMUPARAM 0.53
#define SND_STAR_VOLSTEPS 7
#define SND_STAR_STEPSIZE 0.23
#define SND_STAR_TILE 0.850
#define SND_STAR_BRIGHTNESS 0.0031
#define SND_STAR_DARKMATTER 0.400
#define SND_STAR_DISTFADING 0.730
#define SND_STAR_SATURATION 0.750

vec3 snd_starNest(vec3 dir) {
    ...
    vec3 v = vec3(0.0);
    for (int r = 0; r < SND_STAR_VOLSTEPS; r++) {
        vec3 p = from + s * dir * 0.5;
        p = abs(vec3(SND_STAR_TILE) - mod(p, vec3(SND_STAR_TILE * 2.0)));
        float pa, a = pa = 0.0;
        for (int i = 0; i < SND_STAR_ITERATIONS; i++) {
            p = abs(p) / dot(p, p) - SND_STAR_FORMUPARAM;
            a += abs(length(p) - pa);
            pa = length(p);
        }
        float dm = max(0.0, SND_STAR_DARKMATTER - a * a * 0.001);
        a *= a * a;
        if (r > 6) fade *= 1.0 - dm;
        v += fade;
        v += vec3(s, s * s, s * s * s * s) * a * SND_STAR_BRIGHTNESS * fade;
        fade *= SND_STAR_DISTFADING;
        s += SND_STAR_STEPSIZE;
    }
    v = mix(vec3(length(v)), v, SND_STAR_SATURATION);
    v *= vec3(0.45, 0.7, 1.0);
    return clamp(v * 0.01, 0.0, 1.0);
}

const mat3 snd_voidM3 = mat3(
    0.33338, 0.56034,-0.71817,
   -0.87887, 0.32651,-0.15323,
    0.15162, 0.69596, 0.61339
) * 1.93;

float snd_voidTurbulence(vec3 p, float time) {
...
float snd_voidHash(vec3 p) {
    p = fract(p * vec3(443.897, 441.423, 437.195));
    p += dot(p, p.yzx + 19.19);
    return fract((p.x + p.y) * p.z);
}

vec3 snd_skyVoidArena(vec3 dir) {
    ...
    float energy = snd_voidTurbulence(energyPos, time);
    ...
    float fineEnergy = snd_voidTurbulence(finePos, time);
    ...
    for (int layer = 0; layer < 3; layer++) {
        ...
        float star = pow(snd_voidHash(floor(starPos)), 25.0 - float(layer) * 3.0);
        float pulse = sin(time * (20.0 + float(layer) * 5.0) + star * 50.0) * 0.3 + 0.7;
        vec3 starCol = mix(vec3(1.0, 0.06, 0.04), vec3(1.0, 0.18, 0.1), snd_voidHash(floor(starPos) + 77.0));
...
float snd_skyCaveHash(vec3 p) {
    p = fract(p * vec3(443.897, 441.423, 437.195));
    p += dot(p, p.yzx + 19.19);
    return fract((p.x + p.y) * p.z);
}

vec2 snd_skyCaveVoronoi(vec3 p) {
    vec3 b = floor(p);
    vec3 f = fract(p);
    float dMin = 8.0;
    float dMin2 = 8.0;
    ...
    for (int z = -1; z <= 1; z++) {
        ...
        vec3 r = offset - f + snd_skyCaveHash(b + offset);
        float d = dot(r, r);
        if (d < dMin) {
            dMin2 = dMin;
            dMin = d;
...
vec3 snd_skyCrystalCave(vec3 dir) {
    float time = GameTime * 20.0;
    vec3 rd = normalize(dir);
    ...
    vec2 vor = snd_skyCaveVoronoi(crystalPos * 3.0);
    float edge = vor.y - vor.x;
    float crystalGlow = exp(-edge * 8.0) * 1.5;
    float hueShift = snd_skyCaveHash(floor(crystalPos * 3.0)) * 0.3;
    vec3 crystalCol = mix(vec3(0.45, 0.15, 0.75), vec3(0.15, 0.35, 0.85), hueShift);

    vec3 crystalPos2 = rd * 7.0 + vec3(time * 0.6, 0.0, time * 0.4);
    vec2 vor2 = snd_skyCaveVoronoi(crystalPos2 * 6.0);
    float crystalGlow2 = exp(-vor2.x * 12.0) * 0.8;
    vec3 crystalCol2 = mix(vec3(0.6, 0.2, 0.9), vec3(0.2, 0.5, 1.0), snd_skyCaveHash(floor(crystalPos2 * 6.0) + 100.0) * 0.4);
    ...
    float aurora = abs(snd_skyCaveFbm(auroraPos * 1.5));
    ...
    float sparkle = pow(snd_skyCaveHash(floor(sparklePos)), 20.0) * 3.0;
...
float snd_tunnelMap(vec3 p) {
    float d = -length(p.xy - snd_tunnelPath(p.z)) + 1.2 + 0.3 * sin(p.z * 0.4);
    return d;
}

vec3 snd_tunnelNormal(vec3 p) {
    float d = snd_tunnelMap(p);
    vec2 e = vec2(0.01, 0.0);
    vec3 n = d - vec3(
        snd_tunnelMap(p - e.xyy),
        snd_tunnelMap(p - e.yxy),
        snd_tunnelMap(p - e.yyx)
    );
    return normalize(n);
}

float snd_sMax(float a, float b, float k) {
    return log(exp(k*a) + exp(k*b)) / k;
}

float snd_bumpFunction(vec3 p) {
    vec2 c = snd_tunnelPath(p.z);
    ...
    h = snd_sMax(h, 0.5 + 0.5 * sin(p.z * 8.0 * PI), 16.0);
    h *= h;
    h *= h * h;
    return 1.0 - h;
}
...
vec3 bumpNormal(vec3 p, vec3 n, float bumpFactor) {
    ...
    float f = snd_bumpFunction(p);
    float fx = snd_bumpFunction(p - e.xyy);
    float fy = snd_bumpFunction(p - e.yxy);
    float fz = snd_bumpFunction(p - e.yyx);
    float fx2 = snd_bumpFunction(p + e.xyy);
    float fy2 = snd_bumpFunction(p + e.yxy);
    float fz2 = snd_bumpFunction(p + e.yyx);
...
vec3 snd_skyAlienPlanet(vec3 dir) {
    vec3 rd = normalize(dir);
    float time = GameTime * 20.0;
...
const float SND_RS_MAX_DIST = 200.0;
const float SND_RS_SUN_THRESHOLD = 1.0 - (3.0 / 1800.0);
// -z = world north, where the walls spawn; slightly +x so the sun sits a bit to the players' right
const vec3 SND_RS_SUN_DIRECTION = normalize(vec3(1.2, 0.05, -3.0));
const vec3 SND_RS_RINGS_NORMAL = normalize(vec3(4.0, 1.0, 1.0));
const vec3 SND_RS_RINGS_POS = vec3(1000.0, 0.0, 0.0);

float snd_rs_saturate(float x) { return clamp(x, 0.0, 1.0); }
vec3 snd_rs_saturate(vec3 x) { return clamp(x, 0.0, 1.0); }

float snd_rs_random(vec2 st, int seed) {
...
float snd_rs_noise2D(vec2 pixel, int seed) {
    ...
    float trD = snd_rs_getCornerDot(tr, pixel, seed);
...
vec3 snd_rs_calcMapNormal(vec3 pos, float height) {
    ...
    float hZ = height - snd_rs_map(pos + vec3(0.0, 0.0, 0.001));
    vec3 sX = normalize(vec3(0.001, hX, 0.0));
    vec3 sZ = normalize(vec3(0.0, hZ, 0.001));
    return cross(sZ, sX);
}
float snd_rs_getCloudColor(vec2 pos, float time) {
    float cloudOffset = time * 0.1;
    return snd_rs_noise2D(pos * vec2(0.4, 0.2) + cloudOffset, 2)
    + snd_rs_noise2D(pos / 0.6 + cloudOffset, 2) * 0.1;
}
vec3 snd_rs_calcCloudNormal(vec2 pos, float height, float time) {
    float hX = height - snd_rs_getCloudColor(pos + vec2(0.05, 0.0), time);
    float hZ = height - snd_rs_getCloudColor(pos + vec2(0.0, 0.05), time);
    vec3 sX = normalize(vec3(0.05, hX, 0.0));
    vec3 sZ = normalize(vec3(0.0, hZ, 0.05));
    return cross(sZ, sX);
}
vec3 snd_rs_intersectLinePlane(vec3 lp, vec3 lv, vec3 pp, vec3 pn) {
...
vec3 snd_skyRainbowsSunshines(vec3 dir) {
    ...
    float skyDot = dot(SND_RS_UP, rayForward) * 0.5 + 0.5;
    float sunDot = dot(SND_RS_SUN_DIRECTION, rayForward) * 0.5 + 0.5;
