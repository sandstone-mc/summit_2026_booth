...
if (pngKeys.length > 0) {
    ...
    console.log(`[minify] pngquant candidate: ${pngquant}; resolved: ${Bun.which(pngquant) ?? '(not found)'}`)
    if (!Bun.which(pngquant)) {
        ...
        for (const key of pngKeys) {
            ...
            if (stampMatches && tunedHashes[key] === currentHash) {
                tunedCacheHits++
                continue
            }
            toPngquant.push({ diskPath, key })
        }
        console.log(`[minify] pngquant selection: ${includedCount} included, ${toPngquant.length} pending, ${tunedCacheHits} cached; patterns=${JSON.stringify(tuning.include ?? [])}`)
        ...
        if (toPngquant.length > 0) {
            ...
            console.log(`[minify] pngquant run: ${JSON.stringify([pngquant, ...args])}`)
