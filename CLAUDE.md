...
4. When the user puts a TODO comment in front of something that the user has intentionally broken the syntax of, read the TODO comment and address it directly and immediately.
5. Avoid `any` and `unknown` when possible, if that means large refactoring do it.
...
├── datapack_dependencies/  # Smithed deps (zips) + manually added deps (dirs, like summit-dp-core)
...
Smithed datapack dependencies are managed via the `sand` CLI:
- `sand install smithed <library>` — install a Smithed dependency
- `sand uninstall <library>` — remove a dependency
- `sand refresh` — clear & update cached Smithed libraries

Resolved dependencies land in `.sandstone/output/datapack_dependencies/`:
- **Smithed dependencies** — always `.zip` files
- **Manually added dependencies** — directories (e.g. `summit-dp-core/`)

To add a datapack as a local dependency without Smithed, place the unpacked directory (or zip) in `resources/datapack_dependencies/`.
